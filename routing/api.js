...
const { quote } = require('./../scrapping/quote.js')
const { musically } = require("./../scrapping/listdl/musically");
const { chord } = require("./../scrapping/chord")
const { mlstalk, ffstalk } = require("./../scrapping/stalker");
const { Cersex } = require("./../scrapping/cersex");
const { Porn } = require("./../scrapping/porn");
const { Primbon } = require("./../scrapping/primbon");
const { instagramGetUrl } = require("./../scrapping/igdl");
...
router.get("/download/tiktok", apikeyAndLimit, async (req, res) => {
  ...
  try {
    const trsult = await musically(url)
    console.log(trsult)
    if (!trsult instanceof Object) return res.json(loghandler.error);
    res.json({
      status: "200",
      processed: 'Sukses',
      creator: 'ArifzynXD',
 ...
 *@ STALKER
 */
router.get("/stalk/ff", apikeyAndLimit, async (req, res) => {
  try {
    const id = req.query.id;
    if (!id) return res.status(200).json({ message: "masukan id" });
    await ffstalk(id).then((data) => {
      resSukses(res, data);
    });
  } catch (er) {
    res.json(loghandler.error);
...
router.get("/stalk/ml", apikeyAndLimit, async (req, res) => {
  try {
    const id = req.query.id;
    const zoneid = req.query.zoneid;
    if (!id) return res.status(200).json({ message: "masukan id" });
    if (!zoneid) return res.status(200).json({ message: "masukan zone id" });
    await mlstalk(id, zoneid).then((data) => {
      resSukses(res, data);
    });
  } catch (er) {
    res.json(loghandler.error);
 ...
 *@NSFW Anime
 */
router.get("/nsfw/ahegao", apikeyAndLimit, async (req, res) => {
  rest = JSON.parse(fs.readFileSync(__path + "/data/nsfw/ahegao.json"));
  randAhego = rest[Math.floor(Math.random() * rest.length)];
  await getBuffer(randAhego.url)
    .then((buff) => {
      res.type("png").send(buff);
    })
    .catch((er) => {
...
router.get("/nsfw/ass", apikeyAndLimit, async (req, res) => {
  ahego = JSON.parse(fs.readFileSync(__path + "/data/nsfw/ass.json"));
  randAhego = ahego[Math.floor(Math.random() * ahego.length)];
  await getBuffer(randAhego.url)
    .then((buff) => {
      res.type("png").send(buff);
    })
...
router.get("/nsfw/bdsm", apikeyAndLimit, async (req, res) => {
  ahego = JSON.parse(fs.readFileSync(__path + "/data/nsfw/bdsm.json"));
  randAhego = ahego[Math.floor(Math.random() * ahego.length)];
  await getBuffer(randAhego.url)
    .then((buff) => {
      res.type("png").send(buff);
...
router.get("/nsfw/blowjob", apikeyAndLimit, async (req, res) => {
  ahego = JSON.parse(fs.readFileSync(__path + "/data/nsfw/blowjob.json"));
  randAhego = ahego[Math.floor(Math.random() * ahego.length)];
  await getBuffer(randAhego.url)
    .then((buff) => {
      res.type("png").send(buff);
...
router.get("/nsfw/cuckold", apikeyAndLimit, async (req, res) => {
  ahego = JSON.parse(fs.readFileSync(__path + "/data/nsfw/cuckold.json"));
  randAhego = ahego[Math.floor(Math.random() * ahego.length)];
  await getBuffer(randAhego.url)
    .then((buff) => {
      res.type("png").send(buff);
...
router.get("/nsfw/cum", apikeyAndLimit, async (req, res) => {
  ahego = JSON.parse(fs.readFileSync(__path + "/data/nsfw/cum.json"));
  randAhego = ahego[Math.floor(Math.random() * ahego.length)];
  await getBuffer(randAhego.url)
    .then((buff) => {
      res.type("png").send(buff);
...
router.get("/nsfw/eba", apikeyAndLimit, async (req, res) => {
  ahego = JSON.parse(fs.readFileSync(__path + "/data/nsfw/eba.json"));
  randAhego = ahego[Math.floor(Math.random() * ahego.length)];
  await getBuffer(randAhego.url)
    .then((buff) => {
      res.type("png").send(buff);
...
router.get("/nsfw/ero", apikeyAndLimit, async (req, res) => {
  ahego = JSON.parse(fs.readFileSync(__path + "/data/nsfw/ero.json"));
  randAhego = ahego[Math.floor(Math.random() * ahego.length)];
  await getBuffer(randAhego.url)
    .then((buff) => {
      res.type("png").send(buff);
...
router.get("/nsfw/femdom", apikeyAndLimit, async (req, res) => {
  ahego = JSON.parse(fs.readFileSync(__path + "/data/nsfw/femdom.json"));
  randAhego = ahego[Math.floor(Math.random() * ahego.length)];
  await getBuffer(randAhego.url)
    .then((buff) => {
      res.type("png").send(buff);
...
router.get("/nsfw/foot", apikeyAndLimit, async (req, res) => {
  ahego = JSON.parse(fs.readFileSync(__path + "/data/nsfw/foot.json"));
  randAhego = ahego[Math.floor(Math.random() * ahego.length)];
  await getBuffer(randAhego.url)
    .then((buff) => {
      res.type("png").send(buff);
...
router.get("/nsfw/glasses", apikeyAndLimit, async (req, res) => {
  ahego = JSON.parse(fs.readFileSync(__path + "/data/nsfw/glasses.json"));
  randAhego = ahego[Math.floor(Math.random() * ahego.length)];
  await getBuffer(randAhego.url)
    .then((buff) => {
      res.type("png").send(buff);
...
router.get("/nsfw/hentai", apikeyAndLimit, async (req, res) => {
  ahego = JSON.parse(fs.readFileSync(__path + "/data/nsfw/hentai.json"));
  randAhego = ahego[Math.floor(Math.random() * ahego.length)];
  await getBuffer(randAhego.url)
    .then((buff) => {
      res.type("png").send(buff);
...
router.get("/nsfw/jahy", apikeyAndLimit, async (req, res) => {
  ahego = JSON.parse(fs.readFileSync(__path + "/data/nsfw/jahy.json"));
  randAhego = ahego[Math.floor(Math.random() * ahego.length)];
  await getBuffer(randAhego.url)
    .then((buff) => {
      res.type("png").send(buff);
...
router.get("/nsfw/manga", apikeyAndLimit, async (req, res) => {
  ahego = JSON.parse(fs.readFileSync(__path + "/data/nsfw/manga.json"));
  randAhego = ahego[Math.floor(Math.random() * ahego.length)];
  await getBuffer(randAhego.url)
    .then((buff) => {
      res.type("png").send(buff);
...
router.get("/nsfw/neko", apikeyAndLimit, async (req, res) => {
  ahego = JSON.parse(fs.readFileSync(__path + "/data/nsfw/neko.json"));
  randAhego = ahego[Math.floor(Math.random() * ahego.length)];
  await getBuffer(randAhego.url)
    .then((buff) => {
      res.type("png").send(buff);
...
router.get("/nsfw/loli", apikeyAndLimit, async (req, res) => {
  ahego = JSON.parse(fs.readFileSync(__path + "/data/nsfw/nsfwloli.json"));
  randAhego = ahego[Math.floor(Math.random() * ahego.length)];
  await getBuffer(randAhego.url)
    .then((buff) => {
      res.type("png").send(buff);
...
router.get("/nsfw/orgy", apikeyAndLimit, async (req, res) => {
  ahego = JSON.parse(fs.readFileSync(__path + "/data/nsfw/orgy.json"));
  randAhego = ahego[Math.floor(Math.random() * ahego.length)];
  await getBuffer(randAhego.url)
    .then((buff) => {
      res.type("png").send(buff);
...
router.get("/nsfw/yuri", apikeyAndLimit, async (req, res) => {
  ahego = JSON.parse(fs.readFileSync(__path + "/data/nsfw/yuri.json"));
  randAhego = ahego[Math.floor(Math.random() * ahego.length)];
  await getBuffer(randAhego.url)
    .then((buff) => {
      res.type("png").send(buff);
...
router.get("/nsfw/zettai", apikeyAndLimit, async (req, res) => {
  ahego = JSON.parse(fs.readFileSync(__path + "/data/nsfw/zettai.json"));
  randAhego = ahego[Math.floor(Math.random() * ahego.length)];
  await getBuffer(randAhego.url)
    .then((buff) => {
      res.type("png").send(buff);
...
router.get("/image/jadianime", apikeyAndLimit, async (req, res) => {
  let url = req.query.url
  if (!url) return res.json(loghandler.noturl);
  await getBuffer(`https://api.lolhuman.xyz/api/imagetoanime?apikey=SHADOW-MD&img=${url}`)
    .then((buff) => {
      res.type("png").send(buff);
    })
    .catch((er) => {
 ...
 *@QUOTES
 */
router.get("/quotes/bucin", apikeyAndLimit, async (req, res) => {
  const rest = await getJson('https://raw.githubusercontent.com/ArifzynXD/database/master/kata-kata/bucin.json')
  const randRest = rest[Math.floor(Math.random() * rest.length)];
  resSukses(res, randRest)
})
router.get("/quotes/fakta", apikeyAndLimit, async (req, res) => {
  const rest = await getJson('https://raw.githubusercontent.com/ArifzynXD/database/master/kata-kata/fakta.json')
  const randRest = rest[Math.floor(Math.random() * rest.length)];
  resSukses(res, randRest)
})
router.get("/quotes/gombal", apikeyAndLimit, async (req, res) => {
  const rest = await getJson('https://raw.githubusercontent.com/ArifzynXD/database/master/kata-kata/gombal.json')
  const randRest = rest[Math.floor(Math.random() * rest.length)];
  resSukses(res, randRest)
})
router.get("/quotes/renungan", apikeyAndLimit, async (req, res) => {
  const rest = await getJson('https://raw.githubusercontent.com/ArifzynXD/database/master/kata-kata/renungan.json')
  const randRest = rest[Math.floor(Math.random() * rest.length)];
  resSukses(res, randRest)
})
router.get("/quotes/dilan", apikeyAndLimit, async (req, res) => {
  const rest = await getJson('https://raw.githubusercontent.com/ArifzynXD/database/master/kata-kata/dilan.json')
  const randRest = rest[Math.floor(Math.random() * rest.length)];
  resSukses(res, randRest)
})
router.get("/quotes/faktaunik", apikeyAndLimit, async (req, res) => {
  const rest = await getJson('https://raw.githubusercontent.com/ArifzynXD/database/master/kata-kata/faktaunik.json')
  const randRest = rest[Math.floor(Math.random() * rest.length)];
  resSukses(res, randRest)
})
router.get("/quotes/galau", apikeyAndLimit, async (req, res) => {
  const rest = await getJson('https://raw.githubusercontent.com/ArifzynXD/database/master/kata-kata/galau.json')
  const randRest = rest[Math.floor(Math.random() * rest.length)];
  resSukses(res, randRest)
})
router.get("/quotes/pantun", apikeyAndLimit, async (req, res) => {
  const rest = await getJson('https://raw.githubusercontent.com/ArifzynXD/database/master/kata-kata/pantun.json')
  const randRest = rest[Math.floor(Math.random() * rest.length)];
  resSukses(res, randRest)
})
router.get("/quotes/motivasi", apikeyAndLimit, async (req, res) => {
  const rest = await getJson('https://raw.githubusercontent.com/ArifzynXD/database/master/kata-kata/motivasi.json')
  const randRest = rest[Math.floor(Math.random() * rest.length)];
  resSukses(res, randRest)
})
router.get("/quotes/senja", apikeyAndLimit, async (req, res) => {
  const rest = await getJson('https://raw.githubusercontent.com/ArifzynXD/database/master/kata-kata/senja.json')
  const randRest = rest[Math.floor(Math.random() * rest.length)];
  resSukses(res, randRest)
})
router.get("/quotes/dare", apikeyAndLimit, async (req, res) => {
  const rest = await getJson('https://raw.githubusercontent.com/ArifzynXD/database/master/kata-kata/dare.json')
  const randRest = rest[Math.floor(Math.random() * rest.length)];
  resSukses(res, randRest)
})
router.get("/quotes/truth", apikeyAndLimit, async (req, res) => {
  const rest = await getJson('https://raw.githubusercontent.com/ArifzynXD/database/master/kata-kata/truth.json')
  const randRest = rest[Math.floor(Math.random() * rest.length)];
  resSukses(res, randRest)
})
/*
 @OTHER
 */
router.get("/search/chord", apikeyAndLimit, async (req, res) => {
  let text = req.query.text
  if (!text) return res.json({ message: 'Masukan Query Chord' })
  await chord(text).then(rest => {
    resSukses(res, rest)
  })
    .catch((er) => {
      res.json(loghandler.error);
...
router.get("/maker/quote", apikeyAndLimit, async (req, res) => {
  const text = req.query.text
  const name = req.query.name
  const avatar = req.query.avatar
  if (!text) return res.json({ message: "Masukan Text" })
  if (!name) return res.json({ message: "Masukan Nama" })
  if (!avatar) return res.json({ message: "Url Avatar Note Found" })
  await quote(text, name, avatar).then((buff) => {
    res.type("png").send(buff);
  })
    .catch((er) => {
      res.json(loghandler.error);
