  ...
  await jwt.verify(id, jwtToken, async function (error, decoded) {
    if (error) {
      ...
      if (!tokenize.includes(decoded.email)) {
        ...
        console.log(
          `[ ${time.tz("Asia/Jakarta").format("HH:mm")} ] Email: ${
            decoded.email
          } Success Verified`
        );
...
router.get("/profile", isAuthenticated, async (req, res) => {
  ...
  let identifikasiMember = "Powered By ArifzynXD";
  res.render("profile", {
...
router.post("/register", async (req, res, next) => {
  try {
    ...
    return validasi(req, res, "API ArifzynXD || VERIFICATION", next);
  } catch (err) {
    console.log(err);
...
router.post("/updates", async (req, res) => {
  ...
  const requests = (status, result) =>
    res.json({ status, creator: "ArifzynXD", result });
  if ((email.length || limit.length || expired.length) < 1) {
    ...
    let opts = await global.db.get(`Member_${email}`);
    opts.limit = limit;
    opts.limiter = expired;
    opts.premium = premium.toLowerCase();
    await global.db.set(`Member_${email}`, opts);
    let resulter;
    try {
      resulter = await getJson(
        "https://api.arifzyn.com/api/cekkey?apikey=" + opts.apikey,
        { method: "GET" }
      );
    } catch (er) {
      console.log(er);
...
router.post("/settings", isAuthenticated, async (req, res) => {
  const users = req.user;
  const { username, apikey, email } = req.body;
  let profile;
...
router.post("/forgotpassword", async (req, res) => {
  ...
  if (!users) {
    ...
    await global.db.set(`ResetPw_Member_${email}`, encode);
...
router.get("/newpassword/:token", async (req, res, next) => {
  ...
  const userToken = await global.db.get(
    `ResetPw_Member_${req.parms.email}`,
    token
  );
  if (!userToken || userToken == "done") {
...
router.post("/newpassword/:token", async (req, res) => {
  ...
  if (password.length < 8) {
    ...
    let opts = await global.db.get(`Member_${email}`);
    opts.password = getHashedPassword(password);
    global.db.set(`Member_${email}`, opts);
    global.db.set(`ResetPw_Member_${email}`, "done");
    console.log(
      `[ ${time.tz("Asia/Jakarta").format("HH:mm")} ] Email: ${
        decode.emailUsers
