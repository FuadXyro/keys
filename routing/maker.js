...
router.get("/quotes", apikeyAndLimit, async (req, res) => {
  ...
  await tools
    .qoutes(text, wm, background)
    .then(async (response) => {
      if (!response) return res.json(loghandler.error);
...
router.get("/nulis", apikeyAndLimit, async (req, res) => {
  ...
  if (!text) return res.json(loghandler.nottext);
  await getJson(
    "https://oni-chan.my.id/api/canvas/nulis?text=" + encodeURIComponent(text)
  )
    .then(async (respon) => {
      if (!respon.results) return res.json(loghandler.error);
      await getBuffer(respon.results[0])
        .then((buffer) => {
          res.type("png");
          res.send(buffer);
        })
